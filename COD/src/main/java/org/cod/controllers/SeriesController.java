...
public class SeriesController {
	...
	@GetMapping(value = "/seriesDetailPage/{id}/{startTime}")
	public String movieDetailPage(Model model, @PathVariable Long id,@PathVariable Double startTime,HttpServletRequest request)  {
		Optional<SubSeriesEntity> list = subSeriesRepository.findById(id);

		if (list.isPresent()) {
			SubSeriesEntity subSeriesEntity = list.get();
			if(subSeriesEntity != null) {
				subSeriesEntity.setStartTime(startTime);
			}
